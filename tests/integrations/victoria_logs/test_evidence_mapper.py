...
def test_mapper_records_rows_as_citeable_evidence() -> None:
    ...
    _map_victoria_logs_query(
        evidence,
        {"rows": rows, "query": "level:error"},
        {},
    )

    assert evidence["victoria_logs_query"] == rows
...
def test_mapper_skips_empty_rows() -> None:
    ...
    _map_victoria_logs_query(evidence, {"rows": [], "query": "*"}, {})

    assert "victoria_logs_query" not in evidence
...
def test_merge_tool_evidence_uses_victoria_logs_mapper() -> None:
    ...
    merge_tool_evidence(
        evidence,
        "victoria_logs_query",
        {"rows": [{"_msg": "boom"}], "query": "level:error"},
        {"query": "level:error"},
    )

    assert evidence["victoria_logs_query"] == [{"_msg": "boom"}]
    entries = evidence[CATALOG_ENTRIES_KEY]
    assert entries[0]["source"] == "victoria_logs_query"


def test_repeated_queries_accumulate_under_one_citation_source() -> None:
    evidence: dict[str, Any] = {}

    merge_tool_evidence(
        evidence,
        "victoria_logs_query",
        {"rows": [{"_msg": "first"}], "query": "level:error"},
        {"query": "level:error"},
    )
    merge_tool_evidence(
        evidence,
        "victoria_logs_query",
        {"rows": [{"_msg": "second"}], "query": "level:warn"},
        {"query": "level:warn"},
    )

    assert evidence["victoria_logs_query"] == [
        {"_msg": "first"},
        {"_msg": "second"},
    ]
    entries = evidence[CATALOG_ENTRIES_KEY]
    ...
    assert entries[0]["source"] == "victoria_logs_query"
    assert entries[0]["summary"] == "2 log entries"
    assert entries[0]["snippet"] == "level:error; level:warn"
